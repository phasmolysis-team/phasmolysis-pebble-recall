...
from app.models.users import Users, UpdateUser, UserWithoutPassword, AllPhone
...
async def update_user(
    ...
    statement = select(Users).where(Users.email == current_user.email)
    result = await session.exec(statement)
    user = result.one()
    ...
    if payload.contact_number is not None:
        user.contact_number = AllPhone(payload.contact_number)
